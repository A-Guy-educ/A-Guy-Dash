...
import { getWebOrigin, isDashboardOrigin, requestLogout } from '@/server/aguy-web'
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!isDashboardOrigin(request.headers.get('origin'))) {
  ...
  try {
    ...
    console.error('Shared logout proxy failed', { error, requestId })
